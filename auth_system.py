"""
auth_system.py — Phase 50: Multi-Tenant SaaS
============================================
...
SECRET_KEY = os.environ.get("JWT_SECRET", "openhand_saas_secret_key_123")
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Support ?dev=1 for backend test scripts without updating frontend right away
        if not token and request.args.get("dev") == "1":
            g.user_id = 1
...
def track_usage(user_id, wid, task, score, status, tokens=0):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO workflows (id, user_id, task, score, status) VALUES (?, ?, ?, ?, ?)", 
              (wid, user_id, task, score, status))
    c.execute("UPDATE users SET total_tasks = total_tasks + 1, total_tokens = total_tokens + ? WHERE id = ?", 
              (tokens, user_id))
    conn.commit()
    conn.close()
...
def get_dashboard(user_id):
    ...
    c.execute("SELECT id, task, score, status, timestamp FROM workflows WHERE user_id = ? ORDER BY timestamp DESC LIMIT 10", (user_id,))
    recent = [{"id": r[0], "task": r[1], "score": r[2], "status": r[3], "timestamp": r[4]} for r in c.fetchall()]
    conn.close()
    ...
    successes = sum(1 for w in recent if w["status"] in ("PASSED", "Success", "PARTIAL SUCCESS", "excellent", "good"))
